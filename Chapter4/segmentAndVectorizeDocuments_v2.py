...
def vectorizeDocumentsNgrams(path, ngram_size, tokenizer, labelHash, nb_words_per_segment):
    ...
    for file in files:
        ...
        if match:
            ...
            print('Skipping filename:%s'%(file))
            continue
        (segmented_document,docString)=segmentDocumentNgrams(join(path,file),nb_words_per_segment, ngram_size) 
        segments.extend(segmented_document)
        for segment in segmented_document:
            labels.append(label)
...
def vectorizeDocumentsCharNgrams(path, ngram_size, tokenizer, labelHash, nb_words_per_segment):
    ...
    for file in files:
        ...
        if match:
            ...
            print('Skipping filename:%s'%(file))
            continue
        (segmented_document,docString)=segmentDocumentCharNgrams(join(path,file),nb_words_per_segment, ngram_size)

    ...
    for line in f:
        ...
        if m:
            ...
            if m:
                ...
                if l>max_query_len:
                    max_query_len=l
                v_qs=[np.array(vectorized_question),np.array(vectorized_question)]
...
def pad_data(X,Q,max_story_len, max_query_len):
   ...
   # Q_new=pad_sequences(Q,maxlen=m)
    ...
    for q in Q:
        Q_new.append(pad_sequences(x,maxlen=m))
...
#    tokenizer,vocab_size, max_story_len, max_query_len=create_tokenizer(trainingData,testData)
    ...
    input_facts = Input((max_story_len,m,))
    question = Input((max_story_len,m,))
    ...
    C=Embedding(input_dim=vocab_size,
                output_dim=max_story_len) # max_query_len)
    # B
 ...
 #   input_AO = A(input_facts)
#    print keras.backend.int_shape(input_A),keras.backend.int_shape(input_AO)

 
    input_CO = C(input_facts)
#    input_C=Lambda(embed_C, output_shape=(max_story_len,m, max_query_len))([input_facts])
    input_C=Lambda(embed_C)([input_facts])
    question_B = B(question)
    
    print "I_A",keras.backend.int_shape(input_A)
    print "I_C", keras.backend.int_shape(input_C)
    print "Q_B",keras.backend.int_shape(question_B)


    def multiply(x,n):
        x_prime = K.reshape(x, (-1, n, 1))
        x_transpose = k.transpose(x_prime, perm=[0,2, 1])
        return K.batch_matmul(x_transpose,x_prime)
    
    def lambda_dot(x):
        print"X0", keras.backend.int_shape(x[0])
        print"X1", keras.backend.int_shape(x[1])
        d=dot(x,axes=(2,2)) # add second fact as well
        print"D", keras.backend.int_shape(d)
        return d
    ...
    #input_question_match = Lambda(lambda_dot, output_shape=(max_story_len, max_query_len))([input_A,question_B])
    input_question_match = Lambda(tensor_product, output_shape=(max_story_len, max_query_len))([input_A,question_B])
    print "OK!!!!!!!!!!!!!"
...
#    input_question_match = dot([input_A, question_B], axes=(2, 2))
    ...
    print ">> I_C",keras.backend.int_shape(input_C)
#    Probs=Permute((2, 1))(Probs)  # (samples, max_query_len, max_story_len)
    Probs=Flatten()(Probs)
    input_C=Flatten()(input_C)
    print ">> Probs_perm",keras.backend.int_shape(Probs)
...
#    O = Lambda(lambda x: lambda_sum_prod([x, input_C]),output_shape=(max_story_len,max_query_len))([Probs])
               # (samples, max_story_len, max_query_len)
#    O = Permute((2, 1))(O)  # (samples, max_query_len, max_story_len)
    ...
    print "QB",keras.backend.int_shape(question_B)

    final_match = O # concatenate([O,question_B])
    ...
    #print "W",keras.backend.int_shape(weights)    
    #merged=merge([final_match, weights], mode='mul')
    #answer=Flatten()(merged)
    merged=O
       
    answer = merged #Dense(2)(merged)  # (samples, vocab_size)

...
from keras.layers import recurrent, LSTM, Bidirectional, Input, Dropout,add, dot, concatenate, Activation, Permute, Dense,merge,Reshape,Flatten, Dropout
...
def create_tokenizer(trainingdata, testdata):
    ...
    for line in f:
        ...
        if m:
            ...
            m=re.match("^\d+\s([^\?]+)[\?]\s\t([^\t]+)",line.rstrip())
            # DA m=re.match("^\d+\s([^\?]+)[\?].*",line.rstrip())
            if m:
#              DA  text.append(m.group(1))
                text.append(m.group(1)+' '+m.group(2))
    f.close()
...
#    tokenizer,vocab_size, max_story_len, max_query_len=create_tokenizer(trainingData,testData)
    ...
    for i in range(2):
        print "HOP:",i
        input_C=Dense(size)(input_C) # H, linear mapping
        O = add([Probs, input_C])  # (samples, max_story_len, max_query_len)
        print "OBP",keras.backend.int_shape(O),keras.backend.int_shape(question_B),keras.backend.int_shape(Probs)
        input_question_match = dot([input_A, question_B], axes=(2, 2))

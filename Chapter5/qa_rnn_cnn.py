...
from keras.layers import recurrent, LSTM, Bidirectional, Conv1D, MaxPooling1D, Flatten
...
if __name__=="__main__":
    ...
    X=np.array([np.append(x[0],x[1]) for x in zip(X,Q)])

    X_te=np.array([np.append(x[0],x[1]) for x in zip(X_te,Q_te)])
    
    emb=layers.Embedding(vocab_size,100)
    
    sentence = layers.Input(shape=(max_story_len+max_query_len,), dtype='int32')
    encoded_sentence = emb(sentence)

    encoded_sentence=Conv1D(10,5,padding='valid', activation='relu',strides=1)(encoded_sentence)
    encoded_sentence=MaxPooling1D(pool_size=4)(encoded_sentence)
    
    encoded_sentence = LSTM(100)(encoded_sentence)

    preds = layers.Dense(vocab_size, activation='softmax')(encoded_sentence)

...
def build_vocabulary_v2(vocabulary, sentences):
	# global G.train_words
	print "Generating Vocabulary from the sentences"
	# Count the total number of training words
	G.train_words = 0
...
#	negative_samples = random.sample(range(vocab_size), G.negative)
        negative_samples = random.sample(range(vocab_size), 1)
	while current_word_index in negative_samples:
		negative_samples = random.sample(range(vocab_size), 1) # G.negative)                
...
def pretraining_batch_generator_v3(sentences, vocabulary):
	while True:
		for sentence in sentences:
			...
			sent_seq = [lookup(word,vocabulary) for word in sentence]
			# Create current batch
			sentence_length = len(sent_seq)
					...
					if j == 0:
						...
						if (i+j) < 0 or (i+j) >= sentence_length:
							...
							context_word_indexes.append(sent_seq[(i+j)])
                                                        outS+=" "+sentence[(i+j)]
				# get negative samples
				negative_samples = get_negative_samples(current_word_index,len(vocabulary))
				positive_samples = np.array([context_word_indexes])
                                # flip a coin
                                p=random.uniform(0, 1)
                   
                                if p>0.5:
#                                        print sentence,cw,"==>",outS
...
def generate_samples(sentences, vocabulary, window_size=2):
        vocab_size=len(vocabulary)
   #     rev = dict((v,k) for k, v in vocabulary.iteritems())
        sampling_table = sequence.make_sampling_table(vocab_size)
        while True:
                for sentence in sentences:
                        d=[lookup(w,vocabulary) for w in sentence.split(" ") if w!='']
                        couples, labels = skipgrams(d, vocab_size, window_size=window_size, sampling_table=sampling_table)
                        if len(couples):
                                for i in range(len(couples)):
  #                                      print rev[couples[i][0]],rev[couples[i][1]],"========>",labels[i]
                                        yield [np.array([couples[i][0]]),np.array([couples[i][1]])],[np.array([labels[i]])]
                                        

                                        
def pretraining_batch_generator_v4(sentences, vocabulary):
	while True:
		for sentence in sentences:
			# split the sentence on whitespace
			sentence = sentence.split()
			...
			for i in range(sentence_length-1):
                                cw=sentence[i]
                                current_word_index=np.array([sent_seq[i]])
                                context_word_indexes=np.array([sent_seq[i+1]])
                                outS=sentence[i+1]
				negative_samples = get_negative_samples(current_word_index,len(vocabulary))
				positive_samples = np.array([context_word_indexes])
                                # flip a coin

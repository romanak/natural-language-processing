...
from keras.layers import Input, Dense, Reshape, merge, concatenate, average, Flatten
...
def save_embeddings(save_filepath, weights, nb_docs):
	with codecs.open(save_filepath, "w", "utf-8") as f:
...
def collect_data(textFile,window_size):
...
contexts,docids,targets,vocab=collect_data(sys.argv[1],3) # trigrams
...
model.fit_generator(generator(contexts,targets,100), steps_per_epoch=100, epochs=epochs)

save_embeddings("embedding_doc2vec.txt", embedding_doc.get_weights()[0], len(docids))
